...
void odomHandler(const nav_msgs::Odometry::ConstPtr &odomIn)
{
    ...
    // 车翻了?
    if ((fabs(roll) > inclThre * PI / 180.0 || fabs(pitch) > inclThre * PI / 180.0) && useInclToStop) // useInclToStop = false
    {
        stopInitTime = odomIn->header.stamp.toSec();
    }

    if ((fabs(odomIn->twist.twist.angular.x) > inclRateThre * PI / 180.0 || fabs(odomIn->twist.twist.angular.y) > inclRateThre * PI / 180.0) && useInclRateToSlow) // useInclRateToSlow = false
    {
...
int main(int argc, char **argv)
{
    ...
    while (status) // 10hz
    {
        ...
        if (pathInit)
        {
            // vehicle*Rec为path系原点, 这里应该要把车辆坐标转到path系(与车辆yaw角对齐)
            ...
            float endDis = sqrt(endDisX * endDisX + endDisY * endDisY); // 车辆当前位置距path末端的距离
            ...
            while (pathPointID < pathSize - 1)
            {
                ...
                if (dis < lookAheadDis) // lookAheadDis = 0.5m
                {
                    pathPointID++; // 记录有效path的点数
                }
                else
                {
                    break;
            ...
            disY = path.poses[pathPointID].pose.position.y - vehicleYRel;
            dis = sqrt(disX * disX + disY * disY);
            float pathDir = atan2(disY, disX); // path的方向
            ...
            if (twoWayDrive) // twoWayDrive = true
            {
                double time = ros::Time::now().toSec();
                if (fabs(dirDiff) > PI / 2 && navFwd && time - switchTime > switchTimeThre) // switchTimeThre = 1.0
                {
                    navFwd = false; // 若当前车头与path末端夹角大于90°, 则反向行驶
                    switchTime = time;
                }
                else if (fabs(dirDiff) < PI / 2 && !navFwd && time - switchTime > switchTimeThre)
                {
                    navFwd = true; // 若当前车头与path末端夹角小于90°, 则正向行驶
                    switchTime = time;
                }
            }
            ...
            if (fabs(vehicleSpeed) < 2.0 * maxAccel / 100.0) // maxAccel = 1.0
                vehicleYawRate = -stopYawRateGain * dirDiff;
            else
                vehicleYawRate = -yawRateGain * dirDiff; // yawRateGain = 7.5

            if (vehicleYawRate > maxYawRate * PI / 180.0) // maxYawRate = 45°/s
                vehicleYawRate = maxYawRate * PI / 180.0;
            else if (vehicleYawRate < -maxYawRate * PI / 180.0)
                vehicleYawRate = -maxYawRate * PI / 180.0;

            if (joySpeed2 == 0 && !autonomyMode) // 手动模式
            {
                vehicleYawRate = maxYawRate * joyYaw * PI / 180.0;
            }
            else if (pathSize <= 1 || (dis < stopDisThre && noRotAtGoal)) // stopDisThre = 0.2, noRotAtGoal = true
            {
                vehicleYawRate = 0;
            }
            ...
            if (odomTime < slowInitTime + slowTime && slowInitTime > 0)
                joySpeed3 *= slowRate;

            if (fabs(dirDiff) < dirDiffThre && dis > stopDisThre) // dirDiffThre = 0.1
            {
            ...
            if (pubSkipCount < 0)
            {
                ...
                pubSpeed.publish(cmd_vel); // 发布速度指令
