...
{
    if (!useTerrainAnalysis) // terrainCloudHandler和laserCloudHandler只有一个有效
    {
        laserCloud->clear();
        ...
        for (int i = 0; i < laserCloudSize; i++)
        {
            ...
            if (dis < adjacentRange) // adjacentRange = 3.5
            {
                point.x = pointX;
                point.y = pointY;
                point.z = pointZ;
                laserCloudCrop->push_back(point); // 在感知范围内的点
            }
        }

        laserCloudDwz->clear();
        laserDwzFilter.setInputCloud(laserCloudCrop);
        laserDwzFilter.filter(*laserCloudDwz); // 降采样处理, laserVoxelSize = 0.05m
...
void terrainCloudHandler(const sensor_msgs::PointCloud2ConstPtr &terrainCloud2)
{
    if (useTerrainAnalysis)
    {
        ...
        for (int i = 0; i < terrainCloudSize; i++)
        {
            ...
            float dis = sqrt((pointX - vehicleX) * (pointX - vehicleX) + (pointY - vehicleY) * (pointY - vehicleY));
            if (dis < adjacentRange && (point.intensity > obstacleHeightThre || useCost)) // obstacleHeightThre = 0.2
            {
                point.x = pointX;
                point.y = pointY;
                point.z = pointZ;
        ...
        terrainDwzFilter.filter(*terrainCloudDwz); // 降采样, terrainVoxelSize = 0.2m
...
{
    joyTime = ros::Time::now().toSec(); // 记录每次按键的时间戳
...
void speedHandler(const std_msgs::Float32::ConstPtr &speed)
{
    ...
    if (autonomyMode && speedTime - joyTime > joyToSpeedDelay) // joyToSpeedDelay = 5.0
    {
...
void boundaryHandler(const geometry_msgs::PolygonStamped::ConstPtr &boundary)
{
    ...
    for (int i = 0; i < boundarySize; i++)
    {
        ...
        if (point1.z == point2.z) // 边界要定义在同一平面上
        {
            float disX = point1.x - point2.x;
            float disY = point1.y - point2.y;
            float dis = sqrt(disX * disX + disY * disY);

            int pointNum = int(dis / terrainVoxelSize) + 1; // terrainVoxelSize = 0.2
            for (int pointID = 0; pointID < pointNum; pointID++)
            {
                ...
                for (int j = 0; j < pointPerPathThre; j++) // pointPerPathThre = 2
                {
                    boundaryCloud->push_back(point); // 为什么要同一个点加入两次?
                }
            }
        }
    }
...
void checkObstacleHandler(const std_msgs::Bool::ConstPtr &checkObs)
{
    ...
    if (autonomyMode && checkObsTime - joyTime > joyToCheckObstacleDelay) // joyToCheckObstacleDelay = 5.0
    {
...
int readPlyHeader(FILE *filePtr)
{
    ...
    while (strCur != "end_header")
    {
        ...
        {
            val = fscanf(filePtr, "%d", &pointNum); // pointNum = 101 * 7 = 707
            if (val != 1)
            {
                printf("\nError reading input files, exit.\n\n");
                exit(1);
...
void readStartPaths()
{
    ...
    int pointNum = readPlyHeader(filePtr); // 读取包头
...
void readPaths()
{
    ...
    int pointNum = readPlyHeader(filePtr); // 共7组, 每一组49(7*7)条路径, 每条路径301个点
    ...
    for (int i = 0; i < pointNum; i++)
    {
        ...
        if (pathID >= 0 && pathID < pathNum) // pathNum = 343
        {
            pointSkipCount++;
            if (pointSkipCount > pointSkipNum) // 每隔30个点push一个关键点
            {
...
void readPathList()
{
    ...
    for (int i = 0; i < pathNum; i++)
    {
        ...
        {
            pathList[pathID] = groupID; // 存储每条path对应的group
            endDirPathList[pathID] = 2.0 * atan2(endY, endX) * 180 / PI; // *2?
        }
    }
...
void readCorrespondences() // 读取每个voxel中经过的所有paths的index
{
    ...
    for (int i = 0; i < gridVoxelNum; i++)
    {
        ...
        while (1)
        {
            ...
            if (pathID != -1)
            {
                ...
                {
                    correspondences[gridVoxelID].push_back(pathID); // 当前Voxel中的path index
                }
            }
            else
            {
...
int main(int argc, char **argv)
{
    ...
    ros::Subscriber subLaserCloud = nh.subscribe<sensor_msgs::PointCloud2>("/registered_scan", 5, laserCloudHandler); // VLP-16 Lidar
...
#if PLOTPATHSET == 1
    for (int i = 0; i < pathNum; i++) // 7 * 7 * 7 = 343
    {
        paths[i].reset(new pcl::PointCloud<pcl::PointXYZI>()); // 每条路径由一组点云构成
    }
#endif
    ...
    laserDwzFilter.setLeafSize(laserVoxelSize, laserVoxelSize, laserVoxelSize); // 0.05
    terrainDwzFilter.setLeafSize(terrainVoxelSize, terrainVoxelSize, terrainVoxelSize); // 0.2

    readStartPaths();
#if PLOTPATHSET == 1
    readPaths();
#endif
    readPathList(); // PathList存储每条path的终点
    readCorrespondences();
    ...
    while (status) // 10hz
    {
        ...
        if (newLaserCloud || newTerrainCloud) // LaserCloud和TerrainCloud只有一个有效
        {
            if (newLaserCloud)
            {
                newLaserCloud = false;

                laserCloudStack[laserCloudCount]->clear(); // laserCloudCount = 0
                *laserCloudStack[laserCloudCount] = *laserCloudDwz;
                laserCloudCount = (laserCloudCount + 1) % laserCloudStackNum; // laserCloudStackNum = 1

                plannerCloud->clear();
                for (int i = 0; i < laserCloudStackNum; i++)
                {
                    *plannerCloud += *laserCloudStack[i]; // laserCloudStackNum ≠ 1时, 可实现一个local map?
                }
            }
            ...
            for (int i = 0; i < plannerCloudSize; i++)
            {
                ...
                // 旋转robot系下的点云, 与world系的角度对齐(这里称为path系)
                ...
                if (dis < adjacentRange && ((point.z > minRelZ && point.z < maxRelZ) || useTerrainAnalysis)) // 虑除地面、天花板和adjacentRange以外的点
                {
            ...
            {
                point.x = ((boundaryCloud->points[i].x - vehicleX) * cosVehicleYaw + (boundaryCloud->points[i].y - vehicleY) * sinVehicleYaw) / pathScale; // pathScale = 1.0
            ...
            if (pathRangeBySpeed)
                pathRange = adjacentRange * joySpeed; // 速度越大, 选取的path越长
            if (pathRange < minPathRange) // minPathRange = 1.0
            ...
            if (autonomyMode)
            {
                ...
                relativeGoalDis = sqrt(relativeGoalX * relativeGoalX + relativeGoalY * relativeGoalY); // 距离goal的距离
                joyDir = atan2(relativeGoalY, relativeGoalX) * 180 / PI; // 距离goal的偏角
            }
            ...
            if (pathScale < minPathScale)
                pathScale = minPathScale; // minPathScale = 0.75

            while (pathScale >= minPathScale && pathRange >= minPathRange)
            {
                for (int i = 0; i < 36 * pathNum; i++) // 将轨迹库旋转36份
                {
                ...
                {
                    clearPathPerGroupScore[i] = 0; // 记录每一group轨迹的得分之和
                }

                float minObsAngCW = -180.0;
                float minObsAngCCW = 180.0;
                float diameter = sqrt(vehicleLength / 2.0 * vehicleLength / 2.0 + vehicleWidth / 2.0 * vehicleWidth / 2.0); // 车辆外切圆半径
                float angOffset = atan2(vehicleWidth, vehicleLength) * 180.0 / PI; // 45°

                int plannerCloudCropSize = plannerCloudCrop->points.size();
                for (int i = 0; i < plannerCloudCropSize; i++) // 遍历plannerCloud中的每个点
                {
                    ...
                    {
                        for (int rotDir = 0; rotDir < 36; rotDir++) // 将轨迹库旋转36份, 每两份之间旋转10度
                        {
                            ...
                            if (indX >= 0 && indX < gridVoxelNumX && indY >= 0 && indY < gridVoxelNumY)
                            {
                                ...
                                for (int j = 0; j < blockedPathByVoxelNum; j++)
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        {
                                            pathPenaltyList[pathNum * rotDir + correspondences[ind][j]] = h; // h为该点的强度(强度越高, 是不可跨越障碍物的概率越大)
                                        }
                                    }
                                }
                            }
                ...
                for (int i = 0; i < 36 * pathNum; i++) // 遍历旋转后轨迹库的每一条path
                {
                    ...
                    if (clearPathList[i] < pointPerPathThre) // 认为该轨迹有效, pointPerPathThre = 2
                    {
                        ...
                        // rotDirW表示向侧方延伸的path的权重, 朝向车辆侧方(90°, 270°)的轨迹, 得分低
                        float rotDirW;
                        if (rotDir < 18)
                            rotDirW = fabs(fabs(rotDir - 9) + 1);
                        else
                            rotDirW = fabs(fabs(rotDir - 27) + 1);
                        // 每条path的得分: 与goal的角度偏差越小, 得分越高; 与车辆纵向夹角越小, 得分越高
                        float score = (1 - sqrt(sqrt(dirWeight * dirDiff))) * rotDirW * rotDirW * rotDirW * rotDirW * penaltyScore;
                        if (score > 0)
                        {
                            // 每一组轨迹的总得分
                            clearPathPerGroupScore[groupNum * rotDir + pathList[i % pathNum]] += score; // pathList存储每条path对应的group
                        }
                    }
                }
                ...
                for (int i = 0; i < 36 * groupNum; i++)
                {
                    int rotDir = int(i / groupNum); // rotDir: 0 ~ 36
                    float rotAng = (10.0 * rotDir - 180.0) * PI / 180;
                ...
                if (selectedGroupID >= 0)
                {
                    ...
                    for (int i = 0; i < selectedPathLength; i++) // 遍历startPaths的每个点
                    {
                        ...
                        if (dis <= pathRange / pathScale && dis <= relativeGoalDis / pathScale) // relativeGoalDis: 到goal的距离, 确保不走过
                        {
                            // 将rotDir系的点旋转到path系下
                ...
                if (selectedGroupID < 0) // 未找到可通行path, 缩小pathRange, 进行下一次while循环
                {
            ...
            if (!pathFound) // 没找到可通行path, 发布相应的topic
            {
                path.poses.resize(1);
                ...
                path.header.stamp = ros::Time().fromSec(odomTime);
                path.header.frame_id = "/vehicle";
                pubPath.publish(path); // 发布执行的path, 停留原地, 保证安全
                ...
                freePaths2.header.stamp = ros::Time().fromSec(odomTime);
                freePaths2.header.frame_id = "/vehicle";
                pubFreePaths.publish(freePaths2); // 发布空白的可视化paths
#endif
            }
